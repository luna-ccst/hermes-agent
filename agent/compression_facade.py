...
def _report_compression_timeout(
    ...
    """Report the frozen host observation while preserving upstream cooldown classification."""
    from agent.conversation_compression import mark_context_compression_timed_out, format_compression_timeout
    mark_context_compression_timed_out(agent)
    detail = format_compression_timeout(idle, waited, since_progress, total_ceiling)
    logger.warning("Context compression timed out %s; continuing without compression", detail)
    ...
    if callable(emit):
        emit(f"⚠ Context compression timed out {detail}. No messages were dropped — continuing without "
             "compression. Run /compress to retry, /new for a clean session, or check auxiliary.compression.")
