...
def _restore_compressor_attempt_state(
    ...
    if preserve_timeout_cooldown:
        snapshot = {name: value for name, value in snapshot.items()
                    if name not in (*_COMPRESSOR_COOLDOWN_STATE_FIELDS, "_consecutive_timeout_failures")}
...
class CompressionCommitFence:
    ...
    def __init__(self, total_ceiling_seconds: float | None = None) -> None:
        ...
        # Host-owned: distinguishes timeout cooldowns from explicit fence cancellation.
...
def format_compression_timeout(
    ...
    """
    reason = "total ceiling exhausted" if waited >= ceiling else "idle timeout reached"
    return (
...
def _run_summary_phase(
    ...
    try:
        ...
        try:
            attempt.restore_compressor(
                agent.context_compressor,
                preserve_timeout_cooldown=(commit_fence is not None
                                          and (commit_fence.timeout_observed or commit_fence.deadline_exceeded)
        ...
        host_timeout = (commit_fence is not None
                        and (commit_fence.timeout_observed or commit_fence.deadline_exceeded)
                        and not (hard_cancel_event is not None and hard_cancel_event.is_set()))
        _stall_backoff = not host_timeout and _record_stall_interrupted_backoff(
...
class _Attempt:
    ...
    def restore_compressor(self, compressor: Any, *, preserve_timeout_cooldown: bool = False) -> None:
        ...
        _restore_compressor_attempt_state(
            ...
            preserve_timeout_cooldown=preserve_timeout_cooldown,
        )
...
def compress_context(
    ...
    try:
        ...
        if (commit_fence is not None and commit_fence.is_cancelled
            ...
            attempt.restore_compressor(
                agent.context_compressor,
                preserve_timeout_cooldown=not (_hard_cancel_event is not None and _hard_cancel_event.is_set()),
            )
            _restore_messages_snapshot(messages, messages_before_compression)
            _emit_aborted_attempt_telemetry(agent, attempt.started_at, "commit_fence_cancelled")
            return messages, _existing_system_prompt(agent, system_message)
