...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _channel_gate_allows(
        ...
        if allowed_channels and channel_id not in allowed_channels:
            ...
            return False
        if is_thread_reply:
            standby_key, standby = await self._thread_standby_state(
                channel_id, str(event_thread_ts), user_id, team_id=team_id,
                chat_type="dm" if is_dm else "group")
            if standby:
                if not is_mentioned or not standby_key:
                    return False
                if not await self._clear_thread_standby(standby_key):
                    return False
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        from gateway.thread_quiet_mode import strip_trailing_mode_flag
        original_text, quiet_requested = strip_trailing_mode_flag(original_text)
        ...
        from gateway.thread_quiet_mode import (
            QUIET_MODE_EVENT_KEY, QUIET_MODE_ACTIVE_EVENT_KEY, QUIET_MODE_SESSION_KEY,
        )
        quiet_active = True if quiet_requested is None else quiet_requested
        if quiet_requested is not None:
            msg_event.metadata[QUIET_MODE_EVENT_KEY] = quiet_requested
        else:
            store = getattr(self, "_session_store", None)
            get_metadata = getattr(store, "get_session_metadata", None)
            if callable(get_metadata):
                quiet_active = bool(await asyncio.to_thread(
                    get_metadata, self._event_session_key(msg_event), QUIET_MODE_SESSION_KEY, True))
        if quiet_active:
            msg_event.metadata[QUIET_MODE_ACTIVE_EVENT_KEY] = True
