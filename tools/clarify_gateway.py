...
def get_pending_for_session(session_key: str, *, include_choice_prompts: bool = False, request_id: Optional[str] = None) -> Optional[_ClarifyEntry]:
...
def attempt_text_response_for_session(session_key: str, response: str, *, request_id: Optional[str] = None) -> str:
    """Try to resolve the oldest pending clarify from typed text; returns a TEXT_* outcome."""
    entry = get_pending_for_session(session_key, include_choice_prompts=True, request_id=request_id)
    if entry is None:
