...
conversation is created, including `/new`, `/reset`, and resumption after explicit suspension. If a
...
defaults. In multiplex mode it belongs to the gateway's configuration. For a one-off CLI choice use `hermes chat --model MODEL`.
