...
def _hygiene_compression_timeout_message(
    *, total_exhausted: bool, elapsed: float, idle_timeout: float, progress_observed: bool,
    total_ceiling: Optional[float] = None, since_progress: Optional[float] = None) -> str:
    """Describe the host timeout that actually ended hygiene compression. Chat users cannot edit
    model config, so the copy names chat commands and includes observed timing when available."""
    lead = (
    ...
    if not total_exhausted:
        lead += " If this keeps happening, run `hermes doctor` on the host."
    if total_ceiling is not None and since_progress is not None:
        from agent.conversation_compression import format_compression_timeout
        detail = format_compression_timeout(idle_timeout, elapsed, since_progress, total_ceiling)
        lead += f" Context compression timed out {detail}. No messages were dropped."
    return lead
