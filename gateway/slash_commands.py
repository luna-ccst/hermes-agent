...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if binding is not None and (
            ...
            or binding.get("kind") != "approval"
            or binding.get("session_key") != session_key
            or not isinstance(binding.get("request_id"), str)
            or not binding["request_id"]
        ):
            return t("gateway.approve.no_pending")
        count = resolve_gateway_approval(session_key, choice, resolve_all="all" in args, request_id=binding["request_id"] if binding is not None else None)
        if not count:
            return t("gateway.approve.no_pending")
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        if binding is not None and (
            ...
            return t("gateway.approve.no_pending")
        count = resolve_gateway_approval(session_key, "deny", resolve_all=resolve_all, reason=reason or None, request_id=binding["request_id"] if binding is not None else None)
        if not count:
