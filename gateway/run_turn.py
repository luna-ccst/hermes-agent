...
class GatewayTurnMixin:
    ...
    async def _hmwa_hygiene_on_timeout(self, attempt, hs, session_entry, session_key, source):
        ...
        fence = attempt.commit_fence
        _hyg_waited = time.monotonic() - attempt.wait_started
        _since_progress = fence.seconds_since_progress()
        detail = format_compression_timeout(hs.timeout_seconds, _hyg_waited, _since_progress, hs.total_ceiling_seconds)
        _hyg_total_exhausted = _hyg_waited >= hs.total_ceiling_seconds or fence.deadline_exceeded
        if _hyg_total_exhausted:
            # The worker checks this deadline during summary generation; keep its lease until it exits so
        ...
        _hyg_fence_cancelled = fence.is_cancelled and not _hyg_total_exhausted
        if not _hyg_fence_cancelled:
            fence.timeout_observed = True
        ...
        if _hyg_fence_cancelled:
            ...
            raise
        logger.warning("Session hygiene compression for session %s timed out %s; continuing without compression",
                       session_entry.session_id, detail)
        await self._hmwa_hygiene_notify(
            source, attempt.meta,
            _hygiene_compression_timeout_message(
                total_exhausted=_hyg_total_exhausted, elapsed=_hyg_waited,
                idle_timeout=hs.timeout_seconds, progress_observed=fence.progress_observed,
                total_ceiling=hs.total_ceiling_seconds, since_progress=_since_progress,
            ),
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str, run_generation: int):
        ...
        try:
            ...
            quiet_thread_mode = await resolve_thread_quiet_mode(
                self.async_session_store, session_entry, event, source)
    ...
    def _run_agent_display_settings(self, source: SessionSource, quiet_thread_mode: Optional[bool] = None) -> "GatewayRunner._RunAgentDisplay":
        ...
        policy = resolve_thread_display_policy(quiet_thread_mode)
        if source.platform == Platform.SLACK and quiet_thread_mode is False and policy.tool_progress_mode_override is not None:
            progress_mode = policy.tool_progress_mode_override
        # Webhooks can't edit messages, so tool progress / log mode are off there.
        is_webhook = source.platform == Platform.WEBHOOK
        tool_progress_enabled = policy.tool_progress and progress_mode not in {"off", "log"} and not is_webhook
        ...
        _live_status_adapter = (
            adapter if policy.live_status and getattr(adapter, "supports_status_text", False) and _live_status_mode != "off" else None
        )
        ...
        thinking_mode = _display_surface_mode(
            "thinking_progress", default=False, require_platform_override_for={Platform.MATTERMOST},
        )
        if source.platform == Platform.SLACK and quiet_thread_mode is False and policy.thinking_progress_mode_override is not None:
            thinking_mode = policy.thinking_progress_mode_override
        _thinking_enabled = policy.thinking_progress and thinking_mode != "off"
        ...
        if (
            policy.native_task_cards and source.platform == Platform.SLACK
    ...
    async def _run_agent_queued_followup(
        ...
        try:
            ...
            followup_result = await self._run_agent(
                ...
                quiet_thread_mode=turn_ctx.quiet_mode_enabled,
            )
        except asyncio.CancelledError:
    ...
    async def _run_agent_inner(
        ...
        quiet_thread_mode: Optional[bool] = None,
    ) -> Dict[str, Any]:
        ...
        disp = self._run_agent_display_settings(source, quiet_thread_mode)
