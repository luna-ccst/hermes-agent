...
from gateway.platforms.event import MessageDisposition, MessageEvent, MessageType, ProcessingOutcome
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._command_preflight_handler = None
    ...
    async def _dispatch_inline_reply(self, event: MessageEvent, *, log_cmd: Optional[str] = None) -> MessageDisposition | ProcessingOutcome:
        ...
        return ProcessingOutcome.SUCCESS if result.success else ProcessingOutcome.FAILURE
    ...
    async def _dispatch_active_session_command(self, event: MessageEvent, session_key: str, cmd: str) -> MessageDisposition | ProcessingOutcome:
        ...
        try:
            ...
            if isinstance(outcome, MessageDisposition):
    ...
    async def _dispatch_control_with_receipt(self, event: MessageEvent, session_key: str, cmd=None) -> None:
        outcome = ProcessingOutcome.FAILURE
        try:
            if cmd:
                result = await self._dispatch_active_session_command(event, session_key, cmd)
            else:
                result = await self._dispatch_inline_reply(event)
            outcome = (None if result is MessageDisposition.DEFERRED else
                       ProcessingOutcome.CANCELLED if result is MessageDisposition.REJECTED else result)
        finally:
    ...
    async def _handle_message_while_active(self, event: MessageEvent, session_key: str) -> None:
        ...
        if event.trusted_prompt_reply is not None or should_bypass_active_session(cmd):
            try:
                # /stop, /new, /reset: cancel + response + drain; other bypasses don't cancel.
                if cmd and is_interrupt_then_dispatch(cmd):
                    self._discard_text_debounce(session_key)
                    await self._dispatch_control_with_receipt(event, session_key, cmd)
                else:
                    logger.debug("[%s] Command '/%s' bypassing active-session guard for %s",
                                 self.name, cmd, session_key)
                    await self._dispatch_control_with_receipt(event, session_key)
            except Exception as e:
